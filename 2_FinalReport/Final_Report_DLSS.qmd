...
Our project adopts a context-based approach, an alternative to more content-based methods that require text analysis, leveraging both article relations (internal links) and article structure (sections, citations, length) [-@hanGraphNeuralNetworks2020]. By applying GNNs, we aim to model how signals of reliability and authority propagate through these networks, while complementing them with additional structural features. This network-oriented approach avoids reliance on semantics or style, offering a generalizable and scalable framework for predicting Wikipedia article quality.
...
A better approach would have been to follow the one outlined by [-@cristianconsonniWikiLinkGraphsCompleteLongitudinal2019]. 
