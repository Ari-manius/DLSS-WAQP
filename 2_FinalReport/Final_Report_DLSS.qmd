...
format:
  pdf:
    geometry: top=0.6in, bottom=0.7in, left=0.55in, right=0.55in, includefoot, footskip=10pt
    colorlinks: true
    links-as-notes: true
    header-includes: |
      ...
      % Title block styling (first page)
      \usepackage{titling}
      ...
      \postdate{\begin{figure}[H]\centering\includegraphics[width=.7\textwidth]{Images/cover_image2.png}\end{figure}\end{center}}
      ...
      % Compact paragraphs: no extra vertical gap, small indent
      \AtBeginDocument{%
        \setlength{\parskip}{0pt}%
        \setlength{\parindent}{1em}%
      }

      % ----- TIGHT HEADINGS (KOMA way) -----
      % Remove/near-remove space AFTER headings (no titlesec)
      % Note: a NEGATIVE afterskip makes it run-in.
      ...
      % Run-in subsubsection (heading flows directly into text)
      \RedeclareSectionCommand[
        beforeskip=0.8ex plus .2ex minus .2ex,
        afterskip=-0.5ex
      ]{subsubsection}

       % ----- Heading fonts (classic academic style) -----
      \setkomafont{section}{\itshape\Large}          % big bold serif
      \setkomafont{subsection}{\itshape\normalsize}  % bold serif
      \setkomafont{subsubsection}{\bfseries\normalsize}% italic serif
...
header-includes:
  ...
  - \postdate{\begin{figure}[H]
      \centering
      \includegraphics[width=.8\textwidth]{Images/wiki_network_clean.png}
    \end{figure}
    \end{center}}
...
\begin{figure}[H]
  ...
  \begin{minipage}[b]{0.48\textwidth}
    ...
    \caption{Distribution of Wikipedia article quality classes}
  ...
  \begin{minipage}[b]{0.48\textwidth}
    ...
    \caption{Correlation heatmap and link–reference scatter}
...
\begin{figure}[H]
  ...
  \begin{minipage}[b]{0.48\textwidth}
    ...
    \caption{Pageviews distribution by article quality class}
    \label{fig:pageviews_by_quality}
  \end{minipage}
\end{figure}
...
```{r, echo=FALSE}
network_metrics_tbl <- tribble(
  ~`Feature Category`, ~`Feature Name`, ~`Value`,
  "Basic Structure", "Nodes", 379850.0,
  "Basic Structure", "Edges", 28541137.0,
  "Basic Structure", "Density", 0.00019780995860574628,
  "Connectivity", "Reciprocity", 0.46767909071036656,
  "Local Structure", "Global Clustering", 0.11369373199462422,
  "Distance", "Pseudo-Diameter", 7.0,
)

kable(network_metrics_tbl, caption = "Network structure extracted from graph analysis")
```
...
```{r, echo=FALSE}
assortativity_features_tbl <- tribble(
  ~`Feature Name`, ~`Value`,
  "In-Degree", 0.017541068988285023,
  "Out-Degree", 0.0059696170287088,
  "Categorical Quality", 0.012438923458138865,
  "Categorical Quality (Agg.)", 0.021904663946986307,
  "Numerical Quality", 0.02919179572645144
)

kable(assortativity_features_tbl, caption = "Assortativity features from network analysis")
```
...
\begin{figure}[H]
    ...
    \caption{Complementary cumulative distribution functions for network features by quality category}
    \label{fig:network_features_ccdf}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{2D spectral embedding of Wikipedia articles colored by quality}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth, height=8cm, keepaspectratio]{Images/accuracy_comparison_final.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth, height=8cm, keepaspectratio]{Images/All_Confusion_Matrices_with_RF_ORES.png}
    \caption{Confusion matrices for all models}
    \label{fig:confusion_matrices}
\end{figure}

By contrast, the neural models tend to favor the dominant Low Quality class. They perform reliably on Low Quality articles but often misclassify High Quality ones, leading to weaker recall for this minority group. This is especially visible for GAT, which struggles overall, likely because Wikipedia’s link network does not provide the kind of homophily its attention mechanism relies on.
...
The complete codebase, documentation, and reproducible experiments are available at: <https://github.com/Ari-manius/DLSS-WAQP>
