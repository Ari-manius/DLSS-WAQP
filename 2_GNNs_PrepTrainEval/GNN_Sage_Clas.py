...
from utils.GNN_model import GraphSAGE_Classification
...
model = GraphSAGE_Classification(input_dim, hidden_dim, output_dim).to(device)
...
cosine = CosineAnnealingLR(optimizer, T_max=200 - 10, eta_min=1e-5)
...
weights = class_weights[labels]  # optional

# Pass weights to CrossEntropyLoss
criterion = CrossEntropyLoss(weight=class_weights.to(device))
early_stopper = EarlyStopping(patience=30, min_delta=0.0001, path=f'check/GNN_Sage_checkpoint_{args.data_file}.pt')
...
train_losses, val_losses = train_GNN_model(
    ...
    early_stopper,
    scheduler=scheduler,
    checkpoint_path=f'check/GNN_Sage_checkpoint_{args.data_file}.pt',
    device=device
)
...
plt.ylabel('Loss')
plt.legend()
plt.savefig(f"lossVisual/TrainVal_Loss_GNN_Sage_{args.data_file}.png")
plt.close()